...
from data.data import CnnDmDataset, DebatePediaDataSet
...
class Qsumm_graph(CnnDmDataset):
    """ single article sentence -> single abstract sentence
    (dataset created by greedily matching ROUGE)
    """
    def __init__(self, split, key='nodes_pruned2', subgraph=False):
    ...
    def __getitem__(self, i):
        js_data = super().__getitem__(i)
        query, art_sents, abs_sents, nodes, edges, subgraphs, paras = (
            js_data['query'], js_data['article'], js_data['abstract'], js_data[self.node_key], js_data[self.edge_key], js_data['subgraphs'], js_data['paragraph_merged'])
        #art_sents = [' '.join(art_sents)]
        abs_sents = [' '.join(abs_sents)]
        return query, art_sents, abs_sents, nodes, edges, subgraphs, paras
